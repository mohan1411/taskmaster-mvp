...
const CONTROLLER_PATH = path.join(__dirname, 'backend', 'controllers', 'emailController.js');
const SIMPLE_EXTRACTOR_PATH = path.join(__dirname, 'backend', 'controllers', 'simple-extractor.js');
const BACKUP_PATH = path.join(__dirname, 'backend', 'controllers', 'emailController.js.bak');
...
const functionImplementation = simpleExtractor.match(/const extractTasksFromEmail = async \(req, res\) => \{[\s\S]*?\};/)[0];
