/**
 * Fixed Test Data Generator (Version 2)
 * 
...
async function generateEmails(userId, count) {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      ...
      console.log('Example ObjectId format: 5f7d1d7e95d9d813a7c8a123');
      process.exit(1);
    }
    ...
    for (let i = 0; i < periods.length; i++) {
      ...
      for (let j = 0; j < periodCount; j += batchSize) {
        ...
        for (let k = 0; k < currentBatchSize; k++) {
          ...
          // Create email object - use the userId directly, already validated above
          const email = new Email({
            user: userId,  // Using validated ID directly
